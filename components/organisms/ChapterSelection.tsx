...
import { configureChapters, configureSetup } from "@/lib/features/exam/examSlice";
import type { Exam, ExamSession } from "@/lib/features/exam/examSlice";
import { getAvailableQuestions } from "@/lib/features/exam/examUtils";
...
export function ChapterSelection({ exam, session }: ChapterSelectionProps) {
  ...
  const availableQuestions = useMemo(
    () => getAvailableQuestions(exam, session.selectedChapters),
    [exam, session.selectedChapters],
  );
  ...
  const handleQuestionCountChange = (value: string) => {
    ...
    dispatch(
      configureSetup({
        examId: exam.id,
        questionCount: Number.isNaN(parsed) ? 1 : parsed,
